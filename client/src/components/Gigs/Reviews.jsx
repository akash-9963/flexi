...
function Reviews() {
  ...
  useEffect(() => {
    if (gigData && gigData.reviews.length > 0) {
      const avgRating = gigData.reviews.reduce((sum, { rating }) => sum + rating, 0);
          ...
          <div className="flex flex-col gap-6">
            {gigData.reviews.map((review) => (
              <div className="flex gap-3 border-t pt-6" key={review.id}>
                <div>
                  {review.reviewer?.profileImage ? ( // Optional chaining for safety
                    <Image
