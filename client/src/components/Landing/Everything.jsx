    ...
    <div className="bg-[#f1fdf7] flex py-20 justify-between px-24">
          ...
          {everythingData.map(({ title, subtitle }) => (
            <li key={title}>
              <div className="flex gap-2 items-center text-xl">
                ...
                <h4 className="font-semibold">{title}</h4> {/* Added font-weight for emphasis */}
              </div>
      ...
      <div className="relative h-96 w-2/4">
        <Image
          src="/everything.webp"
          fill
          alt="Illustration showing various service benefits" // More descriptive alt text
          className="object-cover" // Ensure the image fills the space properly
        />
      </div>
    </div>
  );
